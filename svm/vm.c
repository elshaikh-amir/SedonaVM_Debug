...
#include "../jdwp/misc/MConstants.h"

#define VM_WAIT_ONHOLD 1
const static threadid_t APP_TID       = 1;
static volatile u1 isVMSuspended      = FALSE;
static volatile u1 isSingleStep       = FALSE;
static volatile u1 isStepInto         = FALSE;
static volatile u1 suspendCounter     = 0;
static FrameContainer** frames        = NULL;

static size_t frameidCounter          = 1;
static size_t framesAllocated         = 10;
static size_t frames_count            = 0;

// internal forwards
static u1 isVM_Suspended();
static void onVM_Dispose();
static void onVM_Suspend();
static u1 onVM_Exit();
static void onVM_Resume();
static u1 getCurrByteCode();
static size_t getFrames();
static threadid_t getMainPID();
static threadid_t getAppTID();
static Cell* invoke_Method(methodid_t mid, Cell* args, int argc);
static void force_earlyExitMethod();
static void vmExit();
static u1 suspendCount();
static FrameContainer** getAllFrames();

#ifdef __WIN32
inline static void sleep(unsigned long s) {
     Sleep(s);
}
#endif

void init_vmController() {
  VMController->suspendCount = suspendCount;
  VMController->pid = getMainPID;
  VMController->app_tid = getAppTID;
  VMController->sleep = sleep;
  VMController->isSuspended = isVM_Suspended;
  VMController->suspend = onVM_Suspend;
  VMController->force_ExitMethod = force_earlyExitMethod; // PlaceHolder
  VMController->getCurrentByteCode = getCurrByteCode;
  VMController->get_frameCount = getFrames;
  VMController->invoke_Method = invoke_Method;
  VMController->dispose = onVM_Dispose;
  VMController->resume = onVM_Resume;
  VMController->exit = vmExit;
  VMController->getAllFrames = getAllFrames;
  onVM_Suspend();

  frames = (FrameContainer**) malloc(sizeof(FrameContainer) * framesAllocated);
}

inline static FrameContainer** getAllFrames() {
  return frames;
}

inline static Location* newLocation(tagid_t tag, objectid_t clazzid, methodid_t mid, u8 index) {
...
inline static void add_frame(Cell* fp, Location* loc) {
  if(frames_count >= framesAllocated) {
      framesAllocated <<= 1;
      frames = realloc(frames, sizeof(void*) * framesAllocated);
  }
  frames[frames_count] = (FrameContainer*) malloc(sizeof(FrameContainer));
  frames[frames_count]->frameID = frameidCounter++;
  frames[frames_count]->framePointer = fp;
  frames[frames_count]->frameLocation = loc;
  frames_count++;
}

inline static void remove_frame(Cell* fp, size_t id) {
  size_t start = 0;
  while(start < frames_count) {
    if(frames[start]->frameID == id || frames[start]->framePointer == fp) {
      free(frames[start]);
      if(start == frames_count - 1) {
          ...
          memcpy(frames[start], frames[start + 1], frames_count - start);
      }
...
inline static u1 suspendCount() {
  return suspendCounter;
}

inline static void force_earlyExitMethod() {

}

inline static void vmExit() {
  exit(0);
}

inline static Cell* invoke_Method(methodid_t mid, Cell* args, int argc) {
    return NULL;
}

inline static threadid_t getMainPID() {
  return (threadid_t) getpid();
}

inline static threadid_t getAppTID() {
  return APP_TID;
}

inline static u1 getCurrByteCode() {
  return InitArray; // place holder
}

inline static size_t getFrames() {
  return frames_count;
}

// status VM
inline static u1 isVM_Suspended() {
    return suspendCounter != 0;
}

// onVM Dispose
inline static void onVM_Dispose() {
  // varhandler release list of objs
}

// onVM Suspend
inline static void onVM_Suspend() {
    suspendCounter++;
}

// onVM Exit
inline static u1 onVM_Exit(){
    return EXIT_SUCCESS;
}

// onVM Resume
inline static void onVM_Resume(){
    if(suspendCount != 0) {
        suspendCounter--;
    }
}
...
{
    MHandler->onNullPointerException(opcode, &fp, &sp); // push event
...
{
    MHandler->onStackOverFlow(opcode, &fp, &sp); // push event
...
#ifdef VM_DEBUG_MODE
//////////////////////////////////////////////////////////////////////////
// Loop in VM_DEBUG_MODE
//////////////////////////////////////////////////////////////////////////

// These stuff are declared in the scope of vm.c, so later other extern methods can access them
// e.g debug server on events, reading data or setting triggers etc
...
int vmCallDebug(SedonaVM* vm, methodid_t methodid, Cell* args, int argc)
{
    Cell* sp;               // stack pointer
    uint8_t* cp;            // code pointer
    uint8_t* cb;            // code base
    Cell* pp;               // param 0 pointer
    Cell* lp;               // local 0 pointer
    Cell* fp;               // frame pointer
    uint8_t* db;            // static data base
  ...
  if (numParams != argc)
    return ERR_INVALID_METHOD_PARAMS;
  ...
  RefTypeID* mainClass = RefHandler->getStartRef();
  if(getFrames() == 0)
    add_frame(fp, newLocation(TAG_CLASS_OBJECT, mainClass->ref_clazzid, methodid, 0));
  ...
  for (;;)
  {
    ...
    //printf("CodePointer = %i\n", *cp);
#ifndef COMPUTED_GOTO
    // process next opcode using switch if not using computed gotos
    //onEvent_callback(0, 0);

    EventHandler->check_forEvents(*cp, sp, fp, methodid);
    while(isVM_Suspended() == TRUE) {
        while(EventHandler->canDispatchAll() == FALSE) {
            sleep(VM_WAIT_ONHOLD);
        }

        EventHandler->dispatchAll(); // should be only queued waiters
        onVM_Resume();
    }
