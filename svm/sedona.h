...
/*
#if defined BLOCKSIZE_32
    #define SCODE_BLOCK_SIZE 32
    #define block2addr(cb, block) ((cb) + (block<<5))
#elif defined BLOCKSIZE_16
    #define SCODE_BLOCK_SIZE 16
    #define block2addr(cb, block) ((cb) + (block<<4))
#else
...
extern uint8_t* block2addr(const uint8_t* cb, uint16_t block);
extern int setBlockSize(uint16_t blockSize);
extern uint16_t getBlockSize();
...
#ifdef SCODE_DEBUG
    extern int vmCallDebug(SedonaVM* vm, uint16_t method, Cell* args, int argc);
#else
